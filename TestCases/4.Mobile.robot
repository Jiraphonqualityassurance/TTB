*** Settings ***
Resource                 ../ResourcePath/Common_Resource_Mobile.robot



*** Test Cases ***

TSM01 Minimal Todo Application
    [Documentation]       Minimal Todo Application.
    ...                   Focuses on validating the end-to-end flow of task creation
    ...                   and future date/time reminder scheduling.
    [Tags]                TSM01    Success

    Open Minimal Todo Application
    
    AppiumLibrary.Wait Until Element Is Visible    ${BTN_ADD_TODO}    timeout=10s
    Click Element    ${BTN_ADD_TODO}
    
    AppiumLibrary.Hide Keyboard
    Sleep    1s
    Wait Until Element Is Visible    ${TXT_TODO_TITLE}    timeout=5s
    Input Text    ${TXT_TODO_TITLE}    Work
    
    AppiumLibrary.Wait Until Element Is Visible    ${SWITCH_REMIND_ME}    timeout=10s
    Click Element    ${SWITCH_REMIND_ME}    
    
    AppiumLibrary.Wait Until Element Is Visible    ${BTN_DATE_PICKER}    timeout=5s
    Click Element    ${BTN_DATE_PICKER}
    
    Select Target Year Dynamic    ${TARGET_YEAR}

    Select Target Day On Calendar    ${TARGET_DAY}    ${TARGET_MONTH}    ${TARGET_YEAR}
    Click Element    ${DIALOG_BTN_OK}
    
    Select Target Time On Clock Fixed       ${TARGET_HOUR}    ${TARGET_MINUTE}    ${TARGET_AM_PM}

    Click Element    ${BTN_SAVE_TODO}
    
    Wait Until Element Is Visible    ${LBL_TODO_ITEM}    timeout=5s
    AppiumLibrary.Page Should Contain Text    Work

    ${TARGET_TITLE}=             Set Variable    Work
    ${SHORT_MONTH}=              Evaluate    '${TARGET_MONTH}'[:3]
    ${CHECK_MINUTE}=             Evaluate    "{:02d}".format(int('${TARGET_MINUTE}'))

    ${DATE_PART}=    BuiltIn.Set Variable    ${SHORT_MONTH} ${TARGET_DAY}, ${TARGET_YEAR}
    ${TIME_PART}=    BuiltIn.Set Variable    ${TARGET_HOUR}:${CHECK_MINUTE} ${TARGET_AM_PM}

    ${EXPECTED_REMINDER_TEXT}=    BuiltIn.Catenate    SEPARATOR=${SPACE}${SPACE}    ${DATE_PART}    ${TIME_PART}

    ${NEWLY_CREATED_TODO_XPATH}=    BuiltIn.Set Variable    //android.widget.LinearLayout[./android.widget.RelativeLayout//android.widget.TextView[@text="${TARGET_TITLE}"]]//android.widget.TextView[contains(@text, "${EXPECTED_REMINDER_TEXT}")]

    AppiumLibrary.Wait Until Page Contains Element    ${NEWLY_CREATED_TODO_XPATH}    timeout=7s
    AppiumLibrary.Page Should Contain Element         ${NEWLY_CREATED_TODO_XPATH}
    Log    [SUCCESS] บันทึกและตรวจสอบ วันเวลา: ${EXPECTED_REMINDER_TEXT} ถูกต้อง
